...
def market_data():
    ...
    Accepts market parameter: ?market=us or ?market=nse"""
...
def fetch_stock_data(symbol, period='2y'):
    ...
    # Ensure proper suffix for NSE stocks
    if not symbol.endswith('.NS') and symbol + '.NS' in nse_tickers:
        symbol += '.NS'
    elif symbol.endswith('.NS'):
        period = '1y'  # NSE data often has shorter history
    try:
...
def predict():
    try:
        ...
        response = {
            ...
            'symbol': symbol.replace('.NS', '') if market == 'nse' else symbol,
